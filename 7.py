...
class Customer:
    ...
    def __str__(self):
        return (f"Заказчик #{self.__customer_id}: '{self.__name}', "
                f"тел: {self.__phone}, контакт: {self.__contact_person}")

    # Метод для представления объекта (официальная запись)
    def __repr__(self):
        return (f"Customer(customer_id={self.__customer_id}, name='{self.__name}', "
                f"address='{self.__address}', phone='{self.__phone}', "
